...
function generated_cached_data(curr_type,curr_sex,curr_age,curr_race,max_x_axis){
    ...
    for (let year_i=1971;year_i<2021;year_i++){
        let year_data = data[year_i]['incomes']
        ...
        // Get the sample size
        const curr_n = data[year_i]['sample_size']
...
function findGlobalMaxXAndProcess(curr_type,curr_sex,curr_age,curr_race){
    ...
    let total_max = 0;
    for (let year_i=1971;year_i<2021;year_i++){
        let curr_val = data[String(year_i)]['incomes'][1955]
...
function process_data(){
    for (type_i of type_list){
        for (sex_i of sex_list){
            for (age_i of age_list){
                for (race_i of race_list){
