...
let already_built_histogram = false;
...
// Padding to add to the y max of the histogram scales
...
let very_small_height, loader_timeout,display_timeout,density_y_scale,scaled_curve_data,scaled_global_thresholds,density_data, y_axis_ticks_selector, svg_plotting_group,outer_svg,max_histogram_data,mousemove_timeout,wheel_timeout,current_transition_function,scroll_floor,scroll_max,resize_timeout, add_wrapper_timeout,html_sections, html_list_sections,small_width,small_height,tooltip_alignment,svg_width,svg_height,margins,adjusted_width,adjusted_height,histogram_data,x_scale,y_scale,bar_width,plot_wrapper_x,plot_wrapper_y,width_4850,width_150,histogram_y_max;
...
// -- -- CSS -- -- 
// Key is html section. In relation to the html section ([1] means 100% of html section)
// If entry is empty then there is only one css section for the html section
// Length should be num css sections - 1


// -- -- TRANSITION -- -- 
// html-section css-section. 
// Only included if there is a transition function for the microsection (html section css seciton pair).
//  Inputs relative within microsection. 
// Naming is {html_section}-{css_section}
// Maybe speed up with closure so you don't have to select on every iteration ...

// There's only a reverting function if it's necessary. This happens if
// the end/beggining of the transition is carried into the next css section.
// In this case, the transition may get stuck at an incomplete state and mess up adjacent css sections


function revertBarsOpacity(){
    if (this.boomers_bars){
        this.boomers_bars.style('opacity',null);
        this.millennials_bars.style('opacity'.null);
        this.boomers_bars = undefined;
...
const video_structure_dict = {
    ...
    11:{
        ...
        'height':7154,
        'toggleable':false,
        'css_sections': [0.12958,0.28082,0.52712]
    },
    12:{
        'included': true,
        'total_height': 61077,
...
// -- -- CAPTIONS -- -- 
// For now, will revert this later ... 
let captions_sections = [
    0.00253,
    0.00728,
    0.01367,
    0.01882,
    ...
    0.10049,
    0.10455,
    0.11256, 0.11744, 0.12021, 0.12444, 0.13015, 0.13344, 0.13893, 0.14318, 0.14771, 0.14983,
        0.15590,
        0.16152,
        0.16589,
        0.17029,
...
makeCSSSections(); // Adds object of functions for each html section that map to the css section
...
function getGlobalScrollPercentage(){
    const overall_global_scroll_percentage = Math.max(Math.min(content_obj.scrollTop / max_y_scroll, 1),0);
    if (extra_sections_included) return  Math.max(Math.min(content_obj.scrollTop / max_y_scroll, 1),0);
    // 15627 to 44519
    if (overall_global_scroll_percentage < 0.15627) return overall_global_scroll_percentage;
}
...
function toggleStartingVals(next_html_section){
    if (next_html_section==0){
        //document.getElementById('captions-wrapper').style.display = 'none';
        //d3.select('#captions-wrapper').classed('captions-shown',false);
...
// TO DO AT END: WHEN THERE IS TOOLBAR + MOUSE, DOES WHEEL WORK CORRECLY OVER EACH ELMENET?
// HAD ISSUE WITH THAT IN INCOME DISTRIBUTIONS ... 
...
// -- -- -- -- -- -- -- -- -- -- -- 
// -- -- -- -- -- -- -- -- -- -- -- 
// Custom javascript for html sections
// -- -- -- -- -- -- -- -- -- -- -- 
// -- -- -- -- -- -- -- -- -- -- -- 
// -- -- -- -- -- -- -- -- -- -- -- 
...
// -- -- -- -- -- -- -- -- -- -- -- 
// -- -- -- -- -- -- -- -- -- -- -- 
// HTML section 5 - allow scrolling of means
// -- -- -- -- -- -- -- -- -- -- -- 
// -- -- -- -- -- -- -- -- -- -- -- 
function mousedownNum(e) {
    pauseIfPlaying();
    if (has_toolbar){
        document.getElementById('content').addEventListener("scroll",restore_incomes)
    } else{
        window.addEventListener("scroll",restore_incomes)
    }
...
function restore_incomes(){
    ...
    if (has_toolbar){
        document.getElementById('content').removeEventListener("scroll",restore_incomes)
    } else{
        window.removeEventListener("scroll",restore_incomes)
    }
}

// -- -- -- -- -- -- -- -- -- -- -- 
// -- -- -- -- -- -- -- -- -- -- -- 
// HTML section 8 - allow scrolling of medians
// -- -- -- -- -- -- -- -- -- -- -- 
// -- -- -- -- -- -- -- -- -- -- -- 
...
// -- -- -- -- -- -- -- -- -- -- -- 
// -- -- -- -- -- -- -- -- -- -- -- 
// HTML section 8 -  Saving space on html section 10  transition functions - they're all the same function
// -- -- -- -- -- -- -- -- -- -- -- 
// -- -- -- -- -- -- -- -- -- -- -- 
